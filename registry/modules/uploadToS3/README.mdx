...
```bash
npx create-backend-service add upload-to-s3
```
...
To use this module, import the `storeFileS3` router from `@/routes/storeFileS3.js` into your entry point file (e.g., `server.ts`):

```typescript server.ts
import storeFileS3 from "@/routes/storeFileS3.js";
app.use("/store-file", storeFileS3);
```
...
> **NOTE:** Make sure to include the file extension when specifying `fileName`. Without an extension, the file will be stored without one. 
The example above includes the file extension in the file name.
