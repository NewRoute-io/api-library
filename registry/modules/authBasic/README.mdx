...
Import the `auth` router from `@/routes/auth.js` in your main entry point file (e.g., `server.ts`):
...
<PostmanBadge collectionUrl="https://google.com" moduleName="Auth Basic" />
